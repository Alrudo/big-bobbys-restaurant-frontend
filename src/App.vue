<template>
  <div id="app">
    <div id="header">
      <img id="logo" height="280" src="@/assets/img/logo.png" alt="logo">
    </div>
    <div>
      <b-navbar toggleable="lg" class="d-flex justify-content-center" type="dark" variant="dark">
        <b-navbar-toggle target="nav-collapse" class="ml-auto">
          <template #default="{ expanded }">
            <b-icon v-if="expanded" icon="chevron-bar-up"></b-icon>
            <b-icon v-else icon="chevron-bar-down"></b-icon>
          </template>
        </b-navbar-toggle>

        <b-collapse id="nav-collapse" is-nav>
          <b-navbar-nav>
            <b-nav-item>Item 1</b-nav-item>
            <b-nav-item>Item 1</b-nav-item>
            <b-nav-item>Item 1</b-nav-item>
            <b-nav-item>Item 1</b-nav-item>
            <b-nav-item>Item 1</b-nav-item>
            <b-nav-item>Item 1</b-nav-item>
          </b-navbar-nav>
        </b-collapse>
      </b-navbar>
    </div>

<!--    <div id="navbar">-->
<!--      <b-navbar toggleable="lg" class="container justify-content-center" type="dark" variant="transparent">-->
<!--        <b-navbar-toggle target="nav-collapse"></b-navbar-toggle>-->
<!--        <b-collapse id="nav-collapse" is-nav>-->
<!--          <b-navbar-nav>-->
<!--            <b-nav-item router-link to="/">Avaleht</b-nav-item>-->
<!--            <b-nav-item router-link to="/menu">Menüü</b-nav-item>-->
<!--            <b-nav-item router-link to="/cart">Ostukorv</b-nav-item>-->
<!--            <b-nav-item router-link to="/offers">Pakkumised</b-nav-item>-->
<!--            &lt;!&ndash;<b-nav-item router-link to="/gallery">Galerii</b-nav-item>&ndash;&gt;-->
<!--            <b-nav-item router-link to="/contacts">Kontakt</b-nav-item>-->
<!--            <b-nav-item-dropdown v-if="!loggedIn" right>-->
<!--              <template #button-content>-->
<!--                <em>Kasutaja</em>-->
<!--              </template>-->
<!--              <b-dropdown-item router-link to="/register">Registreeri</b-dropdown-item>-->
<!--              <b-dropdown-item router-link to="/login">Logi sisse</b-dropdown-item>-->
<!--            </b-nav-item-dropdown>-->
<!--            <b-nav-item-dropdown v-if="loggedIn" right>-->
<!--              <template #button-content>-->
<!--                &lt;!&ndash; ADD USER NAME INTERPOLATION HERE AFTER LOGIN SYSTEM IN BACK IS IMPLEMENTED &ndash;&gt;-->
<!--                <em>Kasutaja</em>-->
<!--              </template>-->
<!--              <b-dropdown-item router-link to="/profile">Profiil</b-dropdown-item>-->
<!--              <b-dropdown-item @click="loggedIn=false">Logi välja</b-dropdown-item>-->
<!--            </b-nav-item-dropdown>-->
<!--          </b-navbar-nav>-->
<!--        </b-collapse>-->
<!--      </b-navbar>-->
<!--    </div>-->
    <router-view />
  </div>
</template>

<script>
import {BIcon} from "bootstrap-vue";

export default {
  name: 'App',
  data: function () {
    return {
      loggedIn: false,
      items: [
        {
          menu: []
        }
      ]
    }
  },
  components: {
    BIcon
  }
}
</script>

<style>
@import url('https://fonts.googleapis.com/css2?family=Grandstander:ital,wght@1,500&display=swap');
html, body{
  background: #f3e2cc;
  margin: 0;
  padding: 0;
}
#app {
  font-family: Avenir, Helvetica, Arial, sans-serif;
  -webkit-font-smoothing: antialiased;
  -moz-osx-font-smoothing: grayscale;
  text-align: center;
  color: #2c3e50;
  background: #f3e2cc;
}
#header {
  width: 100%;
  height: 300px;
  background: url("assets/img/flower-bg2.jpg");
}
nav.navbar {
  background-image: url(data:image/png;base64,) !important;
  background-color: #190e00 !important;
  border-top: 4px solid #5a2a0e !important;
  box-shadow: 1px 1px 5px #5a2a0e !important;
  /*height: 65px !important;*/
  /*width: 100% !important;*/
  font-family: 'Grandstander', cursive !important;
}
li.nav-item {
  padding-top: 0;
  margin-top: 0;
  color: gold !important;
  font-size: 22px;
  text-shadow: 1px 1px 2px black;
  line-height: 60px;
}
li.nav-item a {
  color: gold !important;
}
#navbar{
  background-image: url(data:image/png;base64,);
  background-color: #190e00;
  border-top: 4px solid #5a2a0e;
  box-shadow: 1px 1px 5px #5a2a0e;
  height: 65px;
  width: 100%;
  font-family: 'Grandstander', cursive;
}
#navbar .container{
  padding-top: 0;
  margin-top: 0;
  color: gold !important;
  font-size: 22px;
  text-shadow: 1px 1px 2px black;
  line-height: 60px;
}
#navbar .container .nav-link{
  padding-top: 0;
  padding-bottom: 0;
  margin-right: 32px;
  color: gold;
}
#navbar .container .nav-link:hover{
  padding-top: 0;
  margin-right: 32px;
  color: #d87300;
  text-shadow: 0 0 5px #000000;
}
#logo{
  margin: 0 auto;
}
.dropdown-menu {
  border-top: none !important;
  border-top-left-radius: 0 !important;
  border-top-right-radius: 0 !important;
}
.navbar-expand .navbar-nav .dropdown-menu {
  background-image: url(data:image/png;base64,);
  background-color: #190e00;
}
.dropdown-item {
  color: gold !important;
}
.dropdown-item:focus, .dropdown-item:hover {
  background-color: transparent !important;
}
.dropdown-item:hover {
  color: #d87300 !important;
}
div#nav-collapse.navbar-collapse.collapse {
  justify-content: center;
  align-items: center;
}
</style>
